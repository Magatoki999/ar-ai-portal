...
def analyze_and_route(user_message: str, sector_info: str, current_time_str: str) -> RouterAnalysis:
    ...
    どのエージェントの思考メモリを優先的に引き出すべきかを調停する
    """
    llm = ChatOpenAI(
        model="gpt-4o-mini", 
        temperature=0.0,  # 調停は決定論的に行うため0に固定
    ...
    router_prompt = ChatPromptTemplate.from_template(
        ...
        "【教授からのメッセージ】\n"
        "「{user_message}」\n\n"
        "思考エージェントの選択基準に則り、最も適切な判定をJSON構造で出力してください。"
    )
    ...
    try:
        ...
        print(f"[Router Error] ルーティング失敗。デフォルト値を返します: {e}")
        return RouterAnalysis(
            intent="chat",
            selected_agents=["pulse", "chronicle"],
