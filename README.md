...
│   └── public/                 # avatar.glb, targets.mind, ruki_appear.wav
...
- **初期挨拶（`[INITIAL_GREETING]`）:** ARマーカー認識時、初回のみ固定音声 `ruki_appear.wav` を最後まで再生してからLLM挨拶を呼ぶ。5分以内の再認識では新たな挨拶を送らない。
...
- **エピソード記憶:** 会話やイベントを `episode_memories` テーブルに保存し、時間軸（今日/昨日/今週/それ以前）でグループ化してプロンプトに織り込む。「覚えて」と言われた場合や、特定キーワード一致時にのみ保存する。
