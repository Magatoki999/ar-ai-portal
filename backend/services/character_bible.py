...
_OPENAI_MODEL = os.getenv("CHARACTER_BIBLE_MODEL") or os.getenv("LLM_MODEL_SMART", "gpt-4o-mini")
# CHARACTER_BIBLE_MODEL が明示的に設定されていればそれを優先、
# 無ければ LLM_MODEL_SMART（Synthesizer用の高精度モデル）を使う。
# どちらも未設定の場合のデフォルトは gpt-4o-mini（コスト最適化方針）。
...
async def generate_mind_profile(year: int | None = None, month: int | None = None) -> str | None:
    ...
    # 2026-07-05追加：将来のAI動画生成（キャスティング素材）に備え、
    # この1ヶ月を映像化するとしたらどんな場面が象徴的かを書いてもらう。
    # データが無い場合は無理に創作せず、正直に「まだ書けない」と書くよう指示する
    # （character_bible.pyのファイル冒頭の設計方針：捏造の禁止に準拠）。
    user_prompt_parts.append(
        "\n## 今月を映像化するなら\n"
        "上記の経験を踏まえて、もしこの1ヶ月の私（ルキルキ）を短い動画にするとしたら、"
        "どの場面が最も象徴的か、1〜2段落で書いてください。光の色・時間帯・構図（近景/遠景など）の"
        "イメージを具体的に添えてください。これは将来、AI動画生成の入力素材として使う想定です。\n"
        "象徴的と言えるほどの経験がまだ無い月は、無理に作らず「まだ映像化できるほどの経験がない」と"
        "正直に書いてください（他のセクションと同じく、捏造は禁止です）。"
    )
