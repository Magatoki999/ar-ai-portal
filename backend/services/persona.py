...
def build_dynamic_constraints(user_call: str, episode_context: str = "") -> str:
    ...
    constraints += (
        ...
        "  ツールの結果を受け取ったら、必ず次のいずれかに従ってください（これは4番の"
        "『リンク出力完全禁止』ルールに対する明示的な例外であり、優先されます）。\n"
        "  - 結果の中に「動画: URL」という行が**含まれていた場合**：\n"
        "    そのURLを一字一句変えずに ||SHOW_VIDEO:URL|| というタグにして、"
        "セリフ末尾に必ず追加してください。この場合にタグを付けないことは禁止です。\n"
        "    URLをセリフの本文中にそのまま書き出すこと（タグを使わずに文章に混ぜること）も"
        "禁止です。必ずタグの中だけに入れてください。\n"
        "  - 結果の中に「動画: URL」の行が**含まれていなかった場合**（プロンプトのみの記録）：\n"
        "    タグは付けず、見つかったプロンプトの内容をそのまま伝えてください。\n"
        "  - ツールが「記録が見当たりません」と答えた場合：\n"
        "    その通り素直に伝えてください（適当なシーンをでっち上げないでください）。\n\n"
        "例（動画URLがある場合。この例のようにタグを省略しないこと）:\n"
        "  'あの祭りの動画、こんな感じでしたよ！||SHOW_VIDEO:https://...||'\n"
    )
