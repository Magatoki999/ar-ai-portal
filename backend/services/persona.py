...
def build_dynamic_constraints(user_call: str, episode_context: str = "") -> str:
    ...
    constraints += (
        "\n【場所名で写真を検索するタグ（||SEARCH_LOCATION_PHOTO||）の使用ルール】\n"
        f"{user_call}さんが「（場所の名前）の写真見せて」のように、特定の場所の名前を挙げて"
        "写真を見たいと言ったときは、まずプロンプト中のエピソードメモリに[image:URL]が"
        "含まれていないか確認してください。\n"
        "- 見つかった場合は ||SHOW_IMAGE:URL|| タグを使ってください（上記ルール通り）。\n"
        "- 見つからない場合（直近の会話には載っていないだけで、過去に記録されている可能性がある場合）は、"
        "「保存されていません」と決めつけずに、必ずセリフ末尾に "
        "||SEARCH_LOCATION_PHOTO:場所名|| タグを追加してください。これによりシステムが過去の記憶"
        "データベース全体を場所名で検索します。\n"
        "- 場所名はユーザーが言った名称そのまま（読み仮名ではなく表記そのまま）を使ってください。\n"
        "例: '「カフェX」の写真、ちょっと探してみますね！||SEARCH_LOCATION_PHOTO:カフェX||'\n"
        "- このタグを使った場合、検索結果は次のメッセージで判明するため、このターンでは"
        "「見つかるかどうか」を確定的に言わないでください（「見つかりませんでした」と先に言ってしまうと"
        "後続の検索結果と矛盾します）。\n"
    )
