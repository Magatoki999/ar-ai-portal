...
async def chat_endpoint(payload: ChatMessage):
    ...
    # ─── 【新設】ルキルキの頭脳に「現在時刻」を完全に同期させるロジック ───
    tokyo_tz = pytz.timezone('Asia/Tokyo')
    now_tokyo = datetime.now(tokyo_tz)
    current_time_str = now_tokyo.strftime("%Y年%m月%d日 %H時%M分") # 分表示バグを%Mで完全修正
    time_context = f"なお、現在の現実世界の日本時間は【{current_time_str}】だよ。"
    ...
    if wallet_address:
        ...
        identity_context = (
            f"【重要】まだ接続が確認できていません。\n"
            f"{time_context}\n"
            f"「んー？まだ同期が不完全かも。ほら、ゲートの認証を通してよ、相棒！」とフランクに促してください。"
        )
    ...
    try:
        if image_base64:
            ...
            # 共通のベースプロンプトからコンテキストを置換展開（Vision機能への統合）
            vision_system_prompt = BASE_SYSTEM_INSTRUCTION.replace("{identity_context}", identity_context)
